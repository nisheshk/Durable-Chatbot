...
export default function ChatInterface() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/chat/send-and-wait', {
        method: 'POST',
        headers: {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          <div className="text-center text-gray-500 mt-8">
            ...
            <p className="text-sm mt-2">Start a conversation and your history will be saved.</p>
          </div>
        )}
          ...
          <div className="flex justify-start">
            <div className="message-bubble bot-message">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-gray-500">AI is thinking... (may take up to 1 minute)</span>
              </div>
            </div>
          </div>
        )}
